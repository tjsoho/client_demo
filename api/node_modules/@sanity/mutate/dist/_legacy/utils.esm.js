import { isKeyedElement, isArrayElement, isPropertyElement, stringify } from "./stringify.esm.js";
...
function upsert(op, currentValue) {
  ...
  if (op.items.forEach((itemToBeUpserted, i) => {
    const existingIndex = currentValue.findIndex(
      (existingItem) => (existingItem == null ? void 0 : existingItem._key) === itemToBeUpserted._key
    );
...
function applyPatchMutation(mutation, document) {
  var _a;
  if ((_a = mutation.options) != null && _a.ifRevision && document._rev !== mutation.options.ifRevision)
