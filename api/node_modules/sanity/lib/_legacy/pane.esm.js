import { Tooltip, TooltipDelayGroupProvider, Button, Dialog, ErrorBoundary, MenuGroup, MenuItem, MenuButton, Tab, Popover as Popover$1, generateHelpUrl } from "./generate-help-url.esm.esm.js";
import "../_singletons.esm.js";
import { jsx, Fragment, jsxs } from "react/jsx-runtime";
import { Box, rem, Flex, Inline, Text, useToast, Card, Stack, Layer, BoundaryElementProvider, MenuDivider, Menu, TextSkeleton, rgba, useElementRect, LayerProvider, _raf2, TextInput, TabList, TabPanel, Code, DialogProvider, Heading, focusFirstDescendant, Container, Spinner, usePortal, PortalProvider, Popover, useLayer, useGlobalKeyDown, useClickOutsideEvent, Grid, Button as Button$1, Dialog as Dialog$1, AvatarStack, Badge, Checkbox, Select } from "@sanity/ui";
...
import { isAfter, addWeeks, isBefore } from "date-fns";
...
function ConfirmDeleteDialog({
  ...
  return /* @__PURE__ */ jsx(
    Dialog,
    {
      ...
      children: /* @__PURE__ */ jsx(DialogBody$1, { children: crossDatasetReferences && internalReferences && !isLoading ? /* @__PURE__ */ jsx(
...
function useDocumentTitle() {
  const { connectionState, schemaType, title, editState } = useDocumentPane(), documentValue = editState?.draft || editState?.published, subscribed = !!documentValue, { error, value } = unstable_useValuePreview({
    enabled: subscribed,
    schemaType,
    value: documentValue
  });
...
}), isLiveEditEnabled = (schemaType) => schemaType.liveEdit === !0, Root$5 = styled(Card)({
  position: "relative",
  zIndex: 1,
  lineHeight: 0
});
...
function useStructureToolSetting(namespace, key, defaultValue) {
  const keyValueStore = useKeyValueStore(), keyValueStoreKey = [STRUCTURE_TOOL_NAMESPACE, namespace, key].filter(Boolean).join("."), value$ = useMemo(() => keyValueStore.getKey(keyValueStoreKey), [keyValueStore, keyValueStoreKey]), value = useObservable(value$, defaultValue), set = useCallback(
    (newValue) => {
...
const VIEW_MODE_PARSED = { id: "parsed", title: "document-inspector.view-mode.parsed" }, VIEW_MODE_RAW = { id: "raw", title: "document-inspector.view-mode.raw-json" }, VIEW_MODES = [VIEW_MODE_PARSED, VIEW_MODE_RAW], lru = new QuickLRU({ maxSize: 1e3 });
...
function Banner(props) {
  ...
  return /* @__PURE__ */ jsx(Card, { borderBottom: !0, paddingX: 4, paddingY: 2, tone, ...rest, children: /* @__PURE__ */ jsxs(Flex, { align: "center", gap: 3, children: [
    ...
    action && /* @__PURE__ */ jsx(Button, { ...action, mode: action.mode || "ghost", tone: action.tone || "default" })
  ] }) });
}
...
function RequestPermissionDialog({
  ...
  const { t } = useTranslation(structureLocaleNamespace), telemtry = useTelemetry(), dialogId = `request-permissions-${useId()}`, projectId = useProjectId(), client = useClient({ apiVersion: "2024-09-26" }), toast = useToast(), zOffset = useZIndex(), [isSubmitting, setIsSubmitting] = useState(!1), [note, setNote] = useState(""), [msgError, setMsgError] = useState(), [hasTooManyRequests, setHasTooManyRequests] = useState(!1), [hasBeenDenied, setHasBeenDenied] = useState(!1), requestedRole$ = useMemo(() => {
    const adminRole = "administrator";
    return !projectId || !client ? of(adminRole) : client.observable.request({ url: `/projects/${projectId}/roles` }).pipe(
      map$1((roles) => roles.filter((role) => role?.appliesToUsers).find((role) => role.name === "editor") ? "editor" : adminRole),
      startWith$1(adminRole),
      catchError$1(() => of(adminRole))
    );
  ...
  return /* @__PURE__ */ jsx(DialogProvider, { position: "fixed", zOffset: zOffset.fullscreen, children: /* @__PURE__ */ jsx(
    Dialog,
    {
      width: 1,
      id: dialogId,
      ...
      children: /* @__PURE__ */ jsx(DialogBody, { children: /* @__PURE__ */ jsxs(Stack, { space: 4, children: [
        /* @__PURE__ */ jsx(Text, { children: t("request-permission-dialog.description.text") }),
        hasTooManyRequests || hasBeenDenied ? /* @__PURE__ */ jsx(Card, { tone: "caution", padding: 3, radius: 2, shadow: 1, children: /* @__PURE__ */ jsxs(Text, { size: 1, children: [
...
const LOADING_STATE = { loading: !0, error: !1, status: void 0 }, EMPTY_STATE = { loading: !1, error: !1, status: "none" }, DECLINED_STATE = { loading: !1, error: !1, status: "declined" }, PENDING_STATE = { loading: !1, error: !1, status: "pending" }, EXPIRED_STATE = { loading: !1, error: !1, status: "expired" }, useRoleRequestsStatus = () => {
  const client = useClient({ apiVersion: "2024-07-01" }), projectId = useProjectId(), checkRoleRequests$ = useMemo(() => !client || !projectId ? of(EMPTY_STATE) : checkRoleRequests(client, projectId), [client, projectId]), { loading, error, status } = useObservable(checkRoleRequests$, LOADING_STATE);
...
function checkRoleRequests(client, projectId) {
  return client.observable.request({
    ...
    catchError((err) => (console.error("Failed to fetch access requests", err), of({ loading: !1, error: !0, status: void 0 }))),
...
function InsufficientPermissionBanner({
  ...
  } = useRoleRequestsStatus(), [requestSent, setRequestSent] = useState(!1), requestPending = useMemo(
    () => roleRequestStatus === "pending" || roleRequestStatus === "declined" || requestSent,
    [roleRequestStatus, requestSent]
  ), currentUserRoles = currentUser?.roles || [], isOnlyViewer = currentUserRoles.length === 1 && currentUserRoles[0].name === "viewer", [showRequestPermissionDialog, setShowRequestPermissionDialog] = useState(!1), listFormat = useListFormat({ style: "short" }), { t } = useTranslation(structureLocaleNamespace), telemetry = useTelemetry(), roleTitles = currentUserRoles.map((role) => role.title), roles = listFormat.formatToParts(roleTitles).map(
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    ...
    showRequestPermissionDialog && /* @__PURE__ */ jsx(
...
function DraftLiveEditBanner({
  ...
  const { t } = useTranslation(structureLocaleNamespace), [isPublishing, setPublishing] = useState(!1), [isDiscarding, setDiscarding] = useState(!1), telemetry = useTelemetry(), { publish, discardChanges } = useDocumentOperation(documentId, displayed?._type || ""), handlePublish = useCallback(() => {
    publish.execute(), setPublishing(!0), telemetry.log(ResolvedLiveEdit, { liveEditResolveType: "publish" });
  }, [publish, telemetry]), handleDiscard = useCallback(() => {
  ...
  }), displayed && displayed._id && !isDraftId(displayed._id) ? null : /* @__PURE__ */ jsx(
    Banner,
    {
      content: /* @__PURE__ */ jsxs(Flex, { align: "center", justify: "space-between", gap: 1, children: [
        /* @__PURE__ */ jsx(Text, { size: 1, weight: "medium", children: /* @__PURE__ */ jsx(
          Translate,
          {
            t,
...
const preventDefault = (ev) => ev.preventDefault(), FormView = forwardRef(function(props, ref) {
  ...
  return /* @__PURE__ */ jsx(
    Container,
    {
      ...
      sizing: "border",
      width: 1,
      children: /* @__PURE__ */ jsx(PresenceOverlay, { margins, children: /* @__PURE__ */ jsx(Box, { as: "form", onSubmit: preventDefault, ref: setRef, children: connectionState === "connecting" && !editState?.draft && !editState?.published ? /* @__PURE__ */ jsx(Delay, { ms: 300, children: /* @__PURE__ */ jsxs(Flex, { align: "center", direction: "column", height: "fill", justify: "center", children: [
      ...
      PortalProvider,
      {
        ...
        children: /* @__PURE__ */ jsx(BoundaryElementProvider, { element: documentScrollElement, children: /* @__PURE__ */ jsxs(
          VirtualizerScrollInstanceProvider,
          {
            ...
            children: [
              activeView.type === "form" && isLiveEdit && ready && /* @__PURE__ */ jsx(
                DraftLiveEditBanner,
                {
                  displayed,
                  documentId,
...
function DocumentHeaderTab(props) {
  const { icon, id, isActive, label, tabPanelId, viewId, ...rest } = props, { ready, editState } = useDocumentPane(), { setView } = usePaneRouter(), handleClick = useCallback(() => setView(viewId), [setView, viewId]);
  return /* @__PURE__ */ jsx(
    Tab,
    {
      ...rest,
...
function DocumentHeaderTitle() {
  ...
  return connectionState === "connecting" && !subscribed ? /* @__PURE__ */ jsx(Fragment, {}) : title ? /* @__PURE__ */ jsx(Fragment, { children: title }) : documentValue ? error ? /* @__PURE__ */ jsx(Fragment, { children: t("panes.document-header-title.error.text", { error: error.message }) }) : /* @__PURE__ */ jsx(Fragment, { children: value?.title || /* @__PURE__ */ jsx("span", { style: { color: "var(--card-muted-fg-color)" }, children: t("panes.document-header-title.untitled.text") }) }) : /* @__PURE__ */ jsx(Fragment, { children: t("panes.document-header-title.new.text", {
...
function DocumentLayout() {
  ...
  return schemaType ? /* @__PURE__ */ jsxs(GetFormValueProvider, { value, children: [
    ...
    /* @__PURE__ */ jsx(FieldActionsProvider, { actions: rootFieldActionNodes, path: EMPTY_ARRAY, children: /* @__PURE__ */ jsxs(
...
const DocumentPaneProvider = memo((props) => {
  const { children, index, pane: pane2, paneKey, onFocusPath } = props, schema = useSchema(), templates = useTemplates(), { setDocumentMeta } = useCopyPaste(), {
    ...
    () => params.path ? fromString(params.path) : EMPTY_ARRAY$2
  ...
  ), ready = connectionState === "connected" && editState.ready && (!params.rev || timelineReady || !!timelineError), displayed = useMemo(
  ...
  const setOpenPath = useCallback(
    ...
    (_nextFocusPath, payload) => {
      ...
      nextFocusPath !== focusPathRef.current && (setFocusPath(pathFor(nextFocusPath)), setOpenPath(pathFor(nextFocusPath.slice(0, -1))), focusPathRef.current = nextFocusPath, onFocusPath?.(nextFocusPath)), updatePresenceThrottled(nextFocusPath, payload);
    },
...
function DocumentListPaneContent(props) {
  const {
    ...
    searchInputElement,
    showIcons
  } = props, schema = useSchema(), { collapsed: layoutCollapsed } = usePaneLayout(), { collapsed, index } = usePane(), [shouldRender, setShouldRender] = useState(!collapsed), { t } = useTranslation(structureLocaleNamespace), handleEndReached = useCallback(() => {
  ...
  const renderItem = useCallback(
    ...
    return /* @__PURE__ */ jsx(RootBox, { overflow: "hidden", height: "fill", $opacity: loadingVariant === "subtle" ? 0.8 : 1, children: /* @__PURE__ */ jsx(CommandListBox, { children: /* @__PURE__ */ jsx(
      CommandList,
      {
        activeItemDataAttr: "data-hovered",
        ariaLabel: paneTitle,
...
const swr = createSWR({ maxSize: 100 });
function listenSearchQuery(options) {
  ...
  return merge(
    ...
    swr(swrKey),
    map$1(({ fromCache, value }) => ({ fromCache, documents: value }))
  );
}
...
function useDocumentList(opts) {
  ...
  }), { enableLegacySearch = !1 } = useWorkspace().search, schema = useSchema(), maxFieldDepth = useSearchMaxFieldDepth(), typeNameFromFilter = useMemo(
    () => findStaticTypesInFilter(searchFilter, paramsProp),
    [searchFilter, paramsProp]
  ), [onRetry$, onRetry] = useMemo(() => observableCallback(), []), [onFetchFullList$, onLoadFullList] = useMemo(() => observableCallback(), []), queryResults$ = useMemo(() => {
...
`, DocumentListPane = memo(function(props) {
  const { childItemId, isActive, pane: pane2, paneKey, sortOrder: sortOrderRaw, layout } = props, schema = useSchema(), { displayOptions, options } = pane2, { apiVersion, filter: filter2 } = options, params = useShallowUnique(options.params || EMPTY_RECORD), typeName = useMemo(() => {
    ...
    [handleClearSearch]
  ), [enableSearchSpinner, setEnableSearchSpinner] = useState();
  useEffect(() => {
...
const PaneContainer = memo(function(props) {
  const { index, isSelected, pane: pane2, paneKey } = props, { name: parentSourceName } = useSource(), {
    ...
    initialValueTemplates = EMPTY_ARRAY$2,
    menuItemGroups,
    menuItems,
    options
  } = pane2, { defaultOrdering = EMPTY_ARRAY$2, filter: filter2 } = options, params = useShallowUnique(options.params || EMPTY_RECORD), sourceName = pane2.source, typeName = useMemo(() => {
    const staticTypes = findStaticTypesInFilter(filter2, params);
    return staticTypes?.length === 1 ? staticTypes[0] : null;
